...
- **患者档案**：唯一档案编号；完整的基础信息、过敏史和既往病史；可按姓名、身份证号、手机号检索；管理员可原子合并重复档案，病历和处方全部转入保留档案，并永久保留档案号、合并人、时间和原因。
...
- `POST /patients/merge-preview`、`POST /patients/merge`、`GET /patient-merge-logs`：管理员预览、执行和查询患者档案合并。
