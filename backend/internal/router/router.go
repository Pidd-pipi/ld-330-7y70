...
func registerAPI(api *gin.RouterGroup, auth *service.AuthService, h Handlers) {
	...
	secured.POST("/admin/patient-merges/preview", middleware.Roles(constants.RoleAdmin), h.Patient.MergePreview)
	secured.POST("/admin/patient-merges", middleware.Roles(constants.RoleAdmin), h.Patient.Merge)
	secured.GET("/admin/patient-merges", middleware.Roles(constants.RoleAdmin), h.Patient.MergeHistory)
